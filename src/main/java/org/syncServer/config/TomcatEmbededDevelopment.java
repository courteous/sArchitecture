...
@Configuration
@Profile("Development")
public class TomcatEmbededDevelopment extends SpringServletContainerInitializer {
	...
	//user this cipher as the one does not have pfs and can be used with wireshark.
		...
		factory.addConnectorCustomizers(new TomcatConnectorCustomizer() {
			...
			public void customize(Connector con) {
				...
				try {
					...
					try {
						con.setAttribute("keystoreFile",
								ResourceUtils.getFile(keystoreFile)
										.getAbsolutePath());
					...
					proto.setProperty("keystoreProvider",
							keystoreProvider.toString());
					proto.setKeyAlias(keystoreAlias);
					

				} catch (Exception ex) {
					throw new IllegalStateException("can't access keystore: ["
							+ "keystore" + "] or truststore: [" + "keystore"
							+ "]", ex);
				}
